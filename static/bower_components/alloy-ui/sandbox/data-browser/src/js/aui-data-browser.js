var Lang = A.Lang,
	...
	isTreeNodeItem = function(node) {
		return node.hasClass('aui-tree-node');
	},
	...
	function expandNode(instance, node, dataSource) {
				...
				callback: {
					success: A.bind(
						function(event) {
							...
							instance._populateTreeView(A.mix(event, { node: node }));
						},
						instance
					),
					failure: A.bind(
...
var DataBrowser = A.Component.create(
	{
		...
		prototype: {
				...
				if (treeView) {
					...
					treeView.on(
						...
						function(event) {
							...
							expandNode(instance, node, dataSource);
						}
					);
				...
				contentBox.delegate(
					...
					function(event) {
						...
						if (isResultItem(node)) {
							...
							if (data) {
								instance.fire('itemSelected', { node: node, item: data });
							}
							else {
			...
			_loadTreeView: function() {
				...
				expandNode(instance, instance._treeView, treeView.dataSource);
			},
			...
			_populateSearchView: function(event) {
				...
				if (ok && !event.error) {
					...
					var fields = (searchView.schema ? searchView.schema.resultFields : null);
